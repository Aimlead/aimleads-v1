...
export async function analyzeLead({ lead, icpProfile, skipLlm = false }) {
  ...
  //   - Low-fit leads (icpScore < llmSkipThreshold): clearly failing ICP — save credits
  const isLowFit = icpScore < aiConfig.llmSkipThreshold;
  ...
  const shouldUseDeepLlm = icpScore >= aiConfig.sonnetScoreThreshold;
  ...
  const [llmSettled, webSettled] = await Promise.allSettled([
    (shouldUseDeepLlm ? analyzeDeep : analyzeQuick)(lead, icpProfile, {
      icp_score: icpScore,
