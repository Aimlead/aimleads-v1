...
app.use((_req, res, next) => {
  res.setHeader('X-AimLeads-Version', buildMetadata.version);
  if (buildMetadata.commitSha) {
    res.setHeader('X-AimLeads-Commit', buildMetadata.commitSha);
  }
  if (buildMetadata.builtAt) {
    res.setHeader('X-AimLeads-Built-At', buildMetadata.builtAt);
  }
  next();
});
