...
import { ICP_GENERATOR_PROMPT_VERSION } from '../services/icpGeneratorService.js';
...
router.post('/generate', icpGenerateLimiter, requireCredits('icp_generate'), validateBody(schemas.icpGenerateSchema), async (req, res) => {
  ...
  const result = await runAiOperation({
    ...
    action: 'icp_generate',
    provider: 'anthropic',
    promptVersion: ICP_GENERATOR_PROMPT_VERSION,
    requestPayload: {
      description_length: description.length,
      description_preview: description.slice(0, 240),
    },
    execute: () => generateIcpFromDescription(description),
  });
  if (!result) {
