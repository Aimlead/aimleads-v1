...
router.post('/:leadId/reanalyze', reanalyzeLimiter, requireCredits('reanalyze_llm'), async (req, res) => {
  ...
  // skipLlm=false for single reanalyze (user-initiated), skipLlm=true for bulk
...
router.post('/:leadId/discover-signals', discoverLimiter, requireCredits('discover_signals'), async (req, res) => {
  ...
  // Run website scraping + Hunter.io email finder + NewsAPI + Claude web research in parallel
  const [discovered, hunterResult, newsFindings, webResearch] = await Promise.all([
  ...
  // Extract signals from news articles using the existing signal extractor
  const newsSignals = extractSignalsFromFindings({ findings: newsFindings, lead });

  // If Hunter found an email and the lead has none, patch it
...
router.post('/:leadId/sequence', sequenceLimiter, requireCredits('sequence'), async (req, res) => {
  ...
  if (!icpProfile) return res.status(400).json({ message: 'No active ICP profile found' });

  // Use existing analysis context from the lead if available
  ...
  const result = await generateOutreachSequence(lead, icpProfile, analysisContext);
  if (!result) {
