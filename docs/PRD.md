...
**À Confirmer :**
Bien que la logique soit implémentée et testée unitairement, la robustesse de ce modèle face à des données réelles et imparfaites doit être validée en production. L'efficacité de la découverte automatique de signaux internet (`discoverInternetSignals`) dépend de la qualité du scan des sites web et de la recherche web de Claude (Anthropic), et nécessite une évaluation qualitative sur un volume significatif de prospects réels.
...
**Existant Confirmé :**
Le système propose des intégrations natives avec HubSpot et Salesforce. L'architecture permet la synchronisation unitaire ou en masse des prospects, ainsi que la configuration du mappage des champs. Une API permet également l'ingestion de signaux externes (`POST /api/leads/:leadId/external-signals`), ouvrant la porte à des automatisations via des outils comme n8n ou Zapier.
