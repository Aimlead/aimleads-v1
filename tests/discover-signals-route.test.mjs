...
const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'aimleads-discover-signals-'));
...
const { default: app } = await import(`../server/app.js?discover-signals=${Date.now()}`);
...
test('discover-signals endpoint extracts findings and re-analyzes', async () => {
  const user = await registerAndGetCookie('discover-user');
  ...
  const icpSave = await request('/icp/active', {
    ...
    body: {
      name: 'Discovery ICP',
      description: 'profile for discover route test',
      weights: {
        industrie: {
  ...
  const response = await request(`/leads/${leadId}/discover-signals`, {
    method: 'POST',
    cookie: user.cookie,
    body: {
      replace: false,
      ...
      findings: [
        {
          title: 'Sigma Labs opens an RFP this week',
          snippet: 'The request for proposal process is now open.',
          url: 'https://sigma.example/rfp-automation',
  ...
  assert.equal(response.payload?.data?.reanalyzed, true);
  assert.ok((response.payload?.data?.signals_count || 0) >= 1);
  assert.ok((response.payload?.data?.extracted_from_findings || 0) >= 1);

  const updatedLead = response.payload?.data?.lead;
