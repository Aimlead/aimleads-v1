...
const apiClient = {
  ...
  leads: {
    ...
    reanalyze: (id) => apiRequest(`/leads/${id}/reanalyze`, { method: 'POST' }),
    discoverSignals: (id, payload = {}) => apiRequest(`/leads/${id}/discover-signals`, { method: 'POST', body: payload }),
    generateSequence: (id) => apiRequest(`/leads/${id}/sequence`, { method: 'POST' }),
  },
  icp: {
    list: () => apiRequest('/icp'),
  ...
  leads: {
    ...
    async reanalyze(id) {
      return runWithMode({
        ...
        apiCall: () => apiClient.leads.reanalyze(id),
        fallbackCall: mockUnsupported,
        passAuthErrors: true,
      });
    },
    ...
    async generateSequence(id) {
      return runWithMode({
        operationName: 'leads.generateSequence',
        apiCall: () => apiClient.leads.generateSequence(id),
        fallbackCall: mockUnsupported,
        passAuthErrors: true,
      });
    },
