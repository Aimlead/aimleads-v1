...
const getStageLabel = (stageId, t) => {
  const labels = {
    to_contact: t('pipeline.stages.toContact', { defaultValue: 'To Contact' }),
    contacted: t('pipeline.stages.contacted', { defaultValue: 'Contacted' }),
    meeting: t('pipeline.stages.meeting', { defaultValue: 'Meeting Set' }),
    proposal: t('pipeline.stages.proposal', { defaultValue: 'Proposal' }),
    won: t('pipeline.stages.won', { defaultValue: 'Won' }),
    lost: t('pipeline.stages.lost', { defaultValue: 'Lost' }),
  };
...
function LeadCard({ lead, onOpen, onStageChange, onDragStart }) {
  ...
  const handleStageChange = async (nextStageId) => {
    ...
    try {
      ...
      toast.success(t('pipeline.toasts.movedTo', { defaultValue: 'Moved to {{stage}}.', stage: getStageLabel(stage.id, t) }));
      onStageChange();
    } catch {
      toast.error(t('pipeline.toasts.updateFailed', { defaultValue: 'Failed to update stage.' }));
    } finally {
      setMoving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 mt-2" onClick={(e) => e.stopPropagation()}>
        {lead.contact_email && (
          <a href={`mailto:${lead.contact_email}`} aria-label={t('pipeline.actions.emailLead', { defaultValue: 'Email lead' })}>
            <Button variant="ghost" size="icon" className="h-6 w-6" aria-label={t('pipeline.actions.emailLead', { defaultValue: 'Email lead' })}>
        ...
        <Select value="" onValueChange={handleStageChange} disabled={moving}>
          <SelectTrigger className="h-6 text-[11px] px-2 py-0 border-slate-200 w-auto">
            <SelectValue placeholder={t('pipeline.actions.moveTo', { defaultValue: 'Move to…' })} />
          </SelectTrigger>
          <SelectContent>
...
export default function Pipeline() {
  ...
  const handleDrop = async (e, targetStageId) => {
    ...
    try {
      ...
      toast.error(t('pipeline.toasts.moveFailed', { defaultValue: 'Failed to move lead.' }));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900">{t('pipeline.title', { defaultValue: 'Pipeline' })}</h1>
        <p className="text-slate-500 mt-1 text-sm">
          {t('pipeline.totalLeads', { defaultValue: '{{count}} total leads across all stages', count: leads.length })}
        </p>
      </div>

      {leads.length === 0 ? (
        <div className="text-center py-20 text-slate-500">
          <p className="text-lg font-semibold">{t('pipeline.empty.title', { defaultValue: 'No leads yet' })}</p>
          <p className="text-sm mt-1">{t('pipeline.empty.body', { defaultValue: 'Import leads from the dashboard to populate your pipeline.' })}</p>
        </div>
      ) : (
        <>
          ...
          <div className="md:hidden">
            <div className="flex items-center justify-between mb-3">
              <button
                ...
                aria-label={t('pipeline.actions.previousStage', { defaultValue: 'Previous stage' })}
              >
                ...
                <div className="flex justify-center gap-1 mt-2">
                  {PIPELINE_STAGES.map((s, i) => (
                    <button
                      ...
                      aria-label={t('pipeline.actions.openStage', { defaultValue: 'Open {{stage}}', stage: getStageLabel(s.id, t) })}
                    />
                  ))}
                </div>
              </div>
              ...
              <button
                ...
                className="p-2 rounded-xl bg-white border border-slate-200 disabled:opacity-30 transition-opacity"
                aria-label={t('pipeline.actions.nextStage', { defaultValue: 'Next stage' })}
              >
            ...
            <div className="space-y-2">
              {mobileStageLeads.length === 0 ? (
                <div className="text-center py-12 text-sm text-slate-400 bg-white rounded-2xl border border-slate-200">
                  {t('pipeline.noLeadsInStage', { defaultValue: 'No leads in this stage' })}
                </div>
              ) : (
                  ...
                  <div className="flex-1 p-2 space-y-2 overflow-y-auto">
                    {stageLeads.length === 0 ? (
                      <div className={cn('text-center py-8 text-xs transition-colors', isDragTarget ? 'text-brand-sky' : 'text-slate-400')}>
                        {isDragTarget ? t('pipeline.dropHere', { defaultValue: 'Drop here' }) : t('pipeline.noLeadsShort', { defaultValue: 'No leads' })}
                      </div>
                    ) : (
                      stageLeads.map((lead) => (
                        <LeadCard
