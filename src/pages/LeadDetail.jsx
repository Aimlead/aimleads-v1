...
import { AlertTriangle, ArrowLeft, Brain, Check, Copy, Database, ExternalLink, Globe, Linkedin, Loader2, Mail, Phone, RefreshCcw, Sparkles, Target } from 'lucide-react';
...
export default function LeadDetail() {
  ...
  const scoreRingValue = Number.isFinite(Number(finalScore)) ? Number(finalScore) : 0;
  const scoreCircumference = 2 * Math.PI * 54;
  const scoreOffset = scoreCircumference - ((Math.max(0, Math.min(100, scoreRingValue)) / 100) * scoreCircumference);
  ...
  const websiteHref = lead.website_url
    ? (/^https?:\/\//i.test(lead.website_url) ? lead.website_url : `https://${lead.website_url}`)
    : '';
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{lead.contact_name || 'Unknown contact'}{lead.contact_role ? ` · ${lead.contact_role}` : ''}</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {lead.final_category && (
                <span className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${categoryStyle(lead.final_category)}`}>
                  {lead.final_category}
                </span>
              )}
              {lead.llm_enriched && (
                <span className="inline-flex items-center gap-1 rounded-full border border-brand-sky/20 bg-brand-sky/5 px-2.5 py-0.5 text-xs font-semibold text-brand-sky">
                  <Brain className="w-3 h-3" />
                  {t('leads.aiSignalsActive')}
                </span>
              )}
              {lead.industry && <span className="rounded-md bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">{lead.industry}</span>}
              {lead.country && <span className="rounded-md bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">{lead.country}</span>}
              {lead.source_list && <span className="rounded-md bg-brand-sky/5 px-2 py-0.5 text-[11px] text-brand-sky">{lead.source_list}</span>}
            </div>
          </div>

          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="gap-1.5" onClick={handleScoreIcp} disabled={scoring || isJobActive}>
              {scoring ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Target className="w-3.5 h-3.5" />}
              {t('leads.scoreIcpBtn')}
            </Button>
            <Button variant="outline" size="sm" className="gap-1.5" onClick={handleAnalyse} disabled={analysing || isJobActive}>
              {analysing ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
              {t('leads.analyseSignalsBtn')}
            </Button>
            <Button size="sm" className="gap-1.5" onClick={() => sequenceMutation.mutate()} disabled={sequenceMutation.isPending || Boolean(sequenceActiveJobId)}>
              {sequenceMutation.isPending || sequenceActiveJobId ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
      ...
      <section className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-950 to-slate-900 p-5 text-white">
        <div className="grid lg:grid-cols-[auto_1fr_auto] gap-6 items-center">
          <div className="relative w-32 h-32">
            <svg className="w-32 h-32 -rotate-90" viewBox="0 0 128 128" aria-hidden="true">
              <circle cx="64" cy="64" r="54" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="10" />
              <circle cx="64" cy="64" r="54" fill="none" stroke="rgb(56 189 248)" strokeWidth="10" strokeDasharray={scoreCircumference} strokeDashoffset={scoreOffset} strokeLinecap="round" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl font-bold leading-none">{finalScore ?? '—'}</span>
              <span className="text-[10px] uppercase tracking-[0.16em] text-slate-300">Final score</span>
            </div>
          </div>

          <div>
            <p className="text-xs uppercase tracking-[0.14em] text-slate-400">Lead scorecard</p>
            <p className="mt-1 text-xl font-semibold">{lead.company_name}</p>
            <p className="text-sm text-slate-300 mt-1">{lead.analysis_summary || lead.icp_summary || t('leads.noAnalysisSummaryYet')}</p>
            <div className="grid sm:grid-cols-3 gap-2 mt-4">
              <div className="rounded-lg border border-white/15 bg-white/5 px-3 py-2">
                <p className="text-[11px] text-slate-300">ICP</p>
                <p className="text-xl font-semibold">{icpScore ?? '—'}</p>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/5 px-3 py-2">
                <p className="text-[11px] text-slate-300">AI</p>
                <p className="text-xl font-semibold">{aiScore ?? '—'}</p>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/5 px-3 py-2">
                <p className="text-[11px] text-slate-300">Boost</p>
                <p className="text-xl font-semibold">{aiBoost ?? '—'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 self-start lg:self-center">
            {email ? (
              <Button variant="secondary" size="icon" asChild className="h-8 w-8"><a href={`mailto:${email}`}><Mail className="w-4 h-4" /></a></Button>
            ) : (
              <Button variant="secondary" size="icon" disabled className="h-8 w-8"><Mail className="w-4 h-4" /></Button>
            )}
            {phone ? (
              <Button variant="secondary" size="icon" asChild className="h-8 w-8"><a href={`tel:${phone}`}><Phone className="w-4 h-4" /></a></Button>
            ) : (
              <Button variant="secondary" size="icon" disabled className="h-8 w-8"><Phone className="w-4 h-4" /></Button>
            )}
            {linkedin ? (
              <Button variant="secondary" size="icon" asChild className="h-8 w-8"><a href={linkedin} target="_blank" rel="noreferrer"><Linkedin className="w-4 h-4" /></a></Button>
            ) : (
              <Button variant="secondary" size="icon" disabled className="h-8 w-8">
                {/* TODO: enable LinkedIn action when URL is available on lead. */}
                <Linkedin className="w-4 h-4" />
              </Button>
            )}
            {websiteHref ? (
              <Button variant="secondary" size="icon" asChild className="h-8 w-8"><a href={websiteHref} target="_blank" rel="noreferrer"><Globe className="w-4 h-4" /></a></Button>
            ) : (
              <Button variant="secondary" size="icon" disabled className="h-8 w-8">
                {/* TODO: enable website action when website is available on lead. */}
      ...
      {currentJob ? (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-brand-sky/20 bg-brand-sky/5 p-3">
          <div className="flex items-center justify-between gap-3 text-sm">
            ...
            <span className="rounded-md border border-brand-sky/20 bg-white px-2 py-1 text-xs font-medium text-brand-sky">{Math.max(0, Math.min(100, Number(currentJob.progress || 0)))}%</span>
          </div>
      ...
      <div className="grid lg:grid-cols-[320px_1fr] gap-4">
        <div className="space-y-4">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">{t('leads.crmNotes')}</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <Select value={followUpStatus} onValueChange={setFollowUpStatus}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {FOLLOW_UP_STATUS_LIST.map((status) => (<SelectItem key={status} value={status}>{status}</SelectItem>))}
                </SelectContent>
              </Select>
              <Textarea placeholder={t('leads.internalNotes')} value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} />
              <Button onClick={handleSave} disabled={saving} className="w-full">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : t('common.save')}</Button>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">{t('leads.leadSnapshot')}</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">
              <p><span className="text-slate-500">{t('common.contact')}: </span>{lead.contact_name || 'N/A'}</p>
              <p><span className="text-slate-500">{t('common.industry')}: </span>{lead.industry || '—'}</p>
              <p><span className="text-slate-500">{t('common.country')}: </span>{lead.country || '—'}</p>
              <p><span className="text-slate-500">{t('common.size')}: </span>{lead.company_size || lead.employee_count || '—'}</p>
              <p><span className="text-slate-500">{t('leads.lastAnalyzedLabel')}: </span>{formatDate(lead.last_analyzed_at, i18n.language) || '—'}</p>
            </CardContent>
          </Card>

          {activeCrmTypes.length > 0 ? (
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Database className="w-4 h-4 text-slate-400" />CRM</CardTitle></CardHeader>
                ...
                {activeCrmTypes.map((crmType) => {
                  ...
                  return (
                    <div key={crmType} className="flex items-center gap-2">
                      <Button size="sm" variant="outline" onClick={() => crmSyncMutation.mutate({ leadId: lead.id, crmType })} disabled={crmSyncMutation.isPending} className="flex-1">
                      ...
                      {lastSync?.crm_object_url ? <a href={lastSync.crm_object_url} target="_blank" rel="noreferrer" className="text-brand-sky"><ExternalLink className="w-3.5 h-3.5" /></a> : null}
                    </div>
                  );
                })}
              </CardContent>
