...
function TemplateCard({ template, onEdit, onDelete, isSelected, onClick }) {
  ...
  const handleCopy = (e) => {
    ...
    setCopied(true);
    toast.success(t('outreach.toasts.templateCopied', { defaultValue: 'Template copied.' }));
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={handleCopy} className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-800 transition-colors px-2 py-1 rounded-lg hover:bg-slate-50" aria-label={t('outreach.actions.copyTemplate', { defaultValue: 'Copy template' })}>
          {copied ? <Check className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3" />}
          {copied ? t('common.copied', { defaultValue: 'Copied!' }) : t('common.copy', { defaultValue: 'Copy' })}
        </button>
        <button onClick={(e) => { e.stopPropagation(); onEdit(template); }} className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-800 transition-colors px-2 py-1 rounded-lg hover:bg-slate-50" aria-label={t('outreach.actions.editTemplate', { defaultValue: 'Edit template' })}>
          <Edit3 className="w-3 h-3" />
          {t('common.edit', { defaultValue: 'Edit' })}
        </button>
        <button onClick={(e) => { e.stopPropagation(); onDelete(template.id); }} className="flex items-center gap-1 text-xs text-slate-500 hover:text-rose-600 transition-colors px-2 py-1 rounded-lg hover:bg-rose-50 ml-auto" aria-label={t('outreach.actions.deleteTemplate', { defaultValue: 'Delete template' })}>
...
export default function Outreach() {
  ...
  const { data: featureFlagsData = null } = useQuery({
    queryKey: ['workspaceFeatureFlags', 'outreach'],
  ...
  const sequenceMutation = useMutation({
    mutationFn: (id) => dataClient.leads.generateSequence(id, { async: asyncJobsEnabled }),
    onSuccess: (data) => {
      if (!data) {
        toast.error(t('outreach.toasts.sequenceUnavailable', { defaultValue: 'Sequence generation is unavailable. Check that an active ICP and ANTHROPIC_API_KEY are configured.' }));
        return;
      }
      if (data?.jobId) {
        setHandledJobId('');
        setActiveJobId(data.jobId);
        toast.success(t('leads.asyncJobQueued'));
        return;
      }
  ...
  useEffect(() => {
    ...
    if (handledJobId === sequenceJob.id) return;

    setHandledJobId(sequenceJob.id);
    setActiveJobId('');
  ...
  const handleSave = (data) => {
    if (isNew) {
      ...
      toast.success(t('outreach.toasts.templateCreated', { defaultValue: 'Template created.' }));
    } else {
      setTemplates((prev) => prev.map((t) => t.id === editing.id ? { ...t, ...data } : t));
      toast.success(t('outreach.toasts.templateUpdated', { defaultValue: 'Template updated.' }));
    }
    setEditing(null);
    setIsNew(false);
  };
      ...
      <Tabs defaultValue={selectedLeadId ? 'sequences' : 'templates'}>
        <TabsList className="bg-slate-100">
          <TabsTrigger value="templates" className="gap-2">
            ...
            {t('outreach.tabs.templates', { defaultValue: 'Templates' })}
          </TabsTrigger>
          <TabsTrigger value="sequences" className="gap-2">
            <Sparkles className="w-4 h-4" />
            {t('outreach.tabs.sequences', { defaultValue: 'AI sequences' })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="templates" className="space-y-5 mt-5">
          <div className="flex items-center justify-between">
            <p className="text-sm text-slate-500">{t('outreach.templatesIntro', { defaultValue: 'Manage your email, LinkedIn, and call scripts. Use {{syntax}} to personalize them.', syntax: '{{variables}}' })}</p>
            <Button
              size="sm"
            ...
            >
              ...
              {t('outreach.actions.newTemplate', { defaultValue: 'New template' })}
            </Button>
          </div>
        ...
        <TabsContent value="sequences" className="mt-5 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button
              ...
              disabled={!selectedLeadId || sequenceMutation.isPending || Boolean(activeJobId)}
          ...
          {sequenceJob && !['completed', 'failed'].includes(sequenceJob.status) && (
            <div className="rounded-2xl border border-brand-sky/20 bg-brand-sky/5 p-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold text-slate-800">{t('leads.asyncJobRunning')}</p>
                  <p className="text-sm text-slate-600">{sequenceJob.message || t('leads.asyncJobQueued')}</p>
                </div>
