...
export default function ResetPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!hasRecoverySession) {
      setError('This reset link is invalid or has already expired. Request a new one.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(submitError?.message || 'Failed to reset password. Please request a new link.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-brand-navy to-slate-900 flex items-center justify-center p-4">
      <Card className="w-full max-w-md border-white/10 bg-white/95 backdrop-blur">
        <CardHeader className="space-y-2">
          ...
          <CardDescription>
            Choose a new password for your account and we&apos;ll sign you back in automatically.
          </CardDescription>
        </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {!hasRecoverySession && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800">
                  This reset link is missing recovery tokens. Request a new email before choosing a new password.
                </div>
              )}

              {recovery.type && recovery.type !== 'recovery' ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500">
                  Recovery link type detected: <span className="font-mono">{recovery.type}</span>
                </div>
              ) : null}
