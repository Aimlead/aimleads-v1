...
export default function LoginModal({ open, onClose, onShowMentions }) {
  ...
  const disabledButtonStyle = {
    ...
    padding: '14px 20px',
    borderRadius: 12,
    border: '1.5px solid rgba(0,31,77,.08)',
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div id={titleId} className="modal-title">{t('landing.loginModal.title')}</div>
        <p id={descriptionId} className="modal-sub">{t('landing.loginModal.subtitle')}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 24 }}>
          ...
          >
            ...
            {t('landing.loginModal.googleSoon')}
          </button>
          ...
          >
            ...
            {t('landing.loginModal.microsoftSoon')}
          </button>
          ...
          >
            {t('landing.loginModal.emailCta')}
          </button>
        </div>

        <p style={{ marginTop: 14, fontSize: 12, color: '#64748b', textAlign: 'center', lineHeight: 1.6 }}>
          {t('landing.loginModal.helpText')}
        </p>

        <p style={{ marginTop: 20, fontSize: 12, color: '#aaa', textAlign: 'center', lineHeight: 1.7 }}>
          {t('landing.loginModal.legalPrefix')}{' '}
          <button
            type="button"
          ...
          >{t('landing.loginModal.terms')}</button> {t('landing.loginModal.and')}{' '}
          <button
            type="button"
            onClick={onShowMentions}
            style={{ color: '#3A8DFF', background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 'inherit', textDecoration: 'underline' }}
          >{t('landing.loginModal.privacy')}</button>.
        </p>
      </div>
    </div>
  );
