      ...
      <div className="h-full px-4 md:px-6 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Link to={ROUTES.accountSettings}>
            <Button variant="ghost" size="icon" title="Account Settings" aria-label="Account Settings" className="w-9 h-9 rounded-xl text-white/40 hover:text-white/80">
              <UserCog className="w-4 h-4" />
            </Button>
          </Link>
          <Button variant="outline" size="sm" onClick={onSignOut} className="text-white/60 border-white/10 hover:border-white/20 rounded-xl text-xs h-8 hidden sm:inline-flex">
            Sign out
          </Button>
        </div>
      </div>
